'''
'스티커'
문제 : 상근이의 여동생 상냥이는 문방구에서 스티커 2n개를 구매했다. 스티커는 그림 (a)와 같이 2행 n열로 배치되어 있다. 
상냥이는 스티커를 이용해 책상을 꾸미려고 한다. 상냥이가 구매한 스티커의 품질은 매우 좋지 않다. 스티커 한 장을 떼면, 
...
입력 : 첫째 줄에 테스트 케이스의 개수 T가 주어진다. 각 테스트 케이스의 첫째 줄에는 n (1 ≤ n ≤ 100,000)이 주어진다. 다음 두 줄에는 n개의 정수가 주어지며, 
...
comment : 규칙만 찾으면 간단한 문제였다.
          (왼쪽 대각선)과 (왼쪽 대각선-1)에서의 값 중 더 큰 것에 현재값을 더해주면 된다.
'''
t = int(input())
for i in range(t):
    n = int(input())
    ...
    if n > 1:
        ...
        for k in range(2, n):
            for j in range(2):
                if j == 0:
                    array[j][k] = max(array[1][k-1], array[1][k-2]) + array[j][k]
                if j == 1:
                    array[j][k] = max(array[0][k-1], array[0][k-2]) + array[j][k]
